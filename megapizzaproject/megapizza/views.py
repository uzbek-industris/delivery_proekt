...
def catalog(request):
    tovars = Tovar.objects.all()  # Получаем все записи модели Tovar из базы данных
    return render(request, 'catalog.html',{'tovars': tovars,'title': 'Каталог -'})
...
def kitchen(request):
    orders = OrderInfo.objects.all().order_by('-id')  # Сортируем заказы по убыванию ID
    return render(request, 'kitchen.html', {'orders': orders, 'title': 'Стол заказов -'})
